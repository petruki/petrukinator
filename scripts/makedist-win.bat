...
if %1 == 1 (echo Making Petrukinator Windows DEMO VERSION distribution ...) else (echo Making Petrukinator Windows FULL VERSION distribution ...)
...
REM - msbuild Petrukinator-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Petrukinator.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Petrukinator.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Petrukinator.aaxplugin\Contents\Win32\Petrukinator.aaxplugin --out .\build-win\aax\bin\Petrukinator.aaxplugin\Contents\Win32\Petrukinator.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Petrukinator.aaxplugin\Contents\x64\Petrukinator.aaxplugin --out .\build-win\aax\bin\Petrukinator.aaxplugin\Contents\x64\Petrukinator.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Petrukinator.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Petrukinator.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Petrukinator Installer" ".\installer\Petrukinator Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Petrukinator Installer.exe" ".\installer\Petrukinator Demo Installer.exe"
REM -del ".\installer\Petrukinator Installer.exe"
REM -)
