...
def  main():
  ...
  installer = "\installer\Petrukinator Installer.exe"
   
  if demo:
    installer = "\installer\Petrukinator Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Petrukinator manual.pdf" 
  ]
  ...
  ZIPNAME = "Petrukinator-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Petrukinator-v" + FULLVERSIONSTR + "-win-demo.zip"
